...
using Random = UnityEngine.Random;
    ...
    public class CarDrivingAIControl : MonoBehaviour
    {
        ...
        private CarDriving m_CarController;    // Reference to actual car controller we are controlling
        ...
        private void Awake()
        {
            ...
            m_CarController = GetComponent<CarDriving>();
        ...
        {
			m_Target = checkPoints [checkPointNum].transform; 

			var forward = transform.TransformDirection (Vector3.forward);
			var target = checkPoints [checkPointNum].transform.position - transform.position;

			if(Vector3.Dot(forward,target) <= 0){
				++checkPointNum;
				if(checkPointNum >= checkPoints.Length)
				{
                    if (gameObject.GetComponentInParent<SensorConfigGA>() != null)
                        gameObject.GetComponentInParent<SensorConfigGA>().EndRun();
                    checkPointNum = 0;
				}
        ...
        private void OnCollisionStay(Collision col)
        {
            ...
            {
                var otherAI = col.rigidbody.GetComponent<CarDrivingAIControl>();
                if (otherAI != null)
                {
