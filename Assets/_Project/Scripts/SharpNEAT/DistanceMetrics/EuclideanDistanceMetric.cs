...
namespace SharpNeat.DistanceMetrics
{
    ...
    public class EuclideanDistanceMetric : IDistanceMetric
    {
        ...
        public double MeasureDistance(float[] p1, float[] p2)
        {
            var arr1 = p1;
            var arr2 = p2;
            ...
            int arr1Idx = 0;
            int arr2Idx = 0;
            double elem1 = arr1[arr1Idx];
            double elem2 = arr2[arr2Idx];
            for (;;)
            {

                // Matching elements. Note that abs() isn't required because we square the result.
                double tmp = elem1 - elem2;
                distance += tmp * tmp;
