...
namespace SharpNeat.Core
{
        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        public double CalcTotalNovelty()
        {
            double total = 0.0;
            foreach(TGenome genome in _genomeList)
            {
                total += genome.EvaluationInfo.Novelty;
            }
            return total;
        }
        ...
        /// <summary>
        /// Calculates the mean novelty of genomes within the specie.
        /// Implemented as a method rather than a property as an indication that this method does significant
        /// work to calculate the value.        
        /// </summary>
        public double CalcMeanNovelty()
        {
            return CalcTotalNovelty() / (double)_genomeList.Count;
        }
