        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        private SpecieStats[] CalcSpecieStats(out int offspringCount)
        {
            ...
            {   
                ...
                inst._meanNovelty = novelty ? _specieList[i].CalcMeanNovelty() : -1;
                totalMeanNovelty = novelty ? totalMeanNovelty + inst._meanNovelty : -1;
            ...
            if(0.0 == totalMeanFitness || (novelty && 0.0 == totalMeanNovelty))
            ...
            else
            {
                ...
                {
                    SpecieStats inst = specieStatsArr[i];
                    double proportion = novelty ? (inst._meanNovelty / totalMeanNovelty) : (inst._meanFitness / totalMeanFitness);
                    inst._targetSizeReal = proportion * (double)_populationSize;
        ...
        private List<TGenome> CreateOffspring(SpecieStats[] specieStatsArr, int offspringCount)
        {
            ...
            for(int i=0; i<specieCount; i++) 
            {
                ...
                // that specie. Fitter / Novel genomes have higher probability of selection.
                ...
                for(int j=0; j<inst._selectionSizeInt; j++) {
                    probabilities[j] = novelty ? genomeList[j].EvaluationInfo.Novelty : genomeList[j].EvaluationInfo.Fitness;
                }
        ...
        {
            // If all genomes have the same fitness/novelty (including zero) then we simply return the first genome.
            TGenome bestGenome = null;
            TGenome bestFitnessGenome = null;
            double bestScore = -1.0;
            double bestFitness = -1.0;
            int bestSpecieIdx = -1;
            int bestFitnessSpeciaIdx = -1;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                double currentScore = novelty ? genome.EvaluationInfo.Novelty : genome.EvaluationInfo.Fitness;
                double currentFitness = genome.EvaluationInfo.Fitness;
                ...
                {
                    bestFitnessGenome = genome;
                    bestFitness = currentFitness;
                    bestFitnessSpeciaIdx = i;
                }
            }

            _currentBestGenome = bestGenome;
            _currentBestFitnessGenome = bestFitnessGenome;
            _bestSpecieIdx = bestSpecieIdx;
            _bestFitnessSpeciaIdx = bestFitnessSpeciaIdx;
        ...
        private void UpdateStats()
        {
            ...
            // Novelty, Fitness and complexity stats.
            double totalNovelty = _genomeList[0].EvaluationInfo.Novelty;
            ...
            for(int i=1; i<count; i++) {
                totalNovelty += _genomeList[i].EvaluationInfo.Novelty;
            ...
            _stats._maxNovelty = _currentBestGenome.EvaluationInfo.Novelty;
            _stats._maxFitness = _currentBestFitnessGenome.EvaluationInfo.Fitness;
            _stats._meanNovelty = totalNovelty / count;
        ...
        private void SortSpecieGenomes()
        {
            ...
            for(int i=0; i<specieCount; i++)
            {
                ...
                if (novelty)
                {
                    _specieList[i].GenomeList.Sort(GenomeNoveltyComparer<TGenome>.Singleton);
                } else
                {
        ...
        /// <summary>
        /// After each generation, novelty scores need to be normalised to the values to make it a relative score
        /// instead of absolute.
        /// </summary>
        private void NormaliseNoveltyScores()
        {
            if (novelty)
            {
                int specieCount = _specieList.Count;

                // Determine Min and Max novelty scores in current generation
                for (int i = 0; i < specieCount; i++)
                {
                    // Get the specie's first genome. Genomes are sorted, therefore this is also the most novel 
                    // genome in the specie. Last has lowest novelty.
                    TGenome bestGenome = _specieList[i].GenomeList[0];
                    TGenome worstGenome = _specieList[i].GenomeList[_specieList[i].GenomeList.Count - 1];

                    if (bestGenome.EvaluationInfo.Novelty > maxNov)
                        maxNov = bestGenome.EvaluationInfo.Novelty;
                    if (worstGenome.EvaluationInfo.Novelty < minNov && worstGenome.EvaluationInfo.Novelty > 0)
                        minNov = worstGenome.EvaluationInfo.Novelty;
                }

                // Normalise
                for (int i = 0; i < specieCount; i++)
                {
                    foreach (TGenome currentGenome in _specieList[i].GenomeList)
                    {
                        double rawNovelty = currentGenome.EvaluationInfo.Novelty;
                        double denominator = (maxNov - minNov);
                        if (denominator == 0 || rawNovelty == 0)
                            continue;

                        double normalised = (rawNovelty - minNov) / denominator;
                        ...
                        currentGenome.EvaluationInfo.SetNovelty(normalised);
                    }
                }
            }
        }
